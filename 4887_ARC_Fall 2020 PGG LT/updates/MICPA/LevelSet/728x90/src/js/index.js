...
(function () {
	...
	nameSpace.init = function () {
		...
		TweenMax.set('#drawer', {x:width})
		TweenMax.set(["#bg-img"], { scale:0.95, x:-10, y:-10});
		TweenMax.set(['#copy-1'], { x: 0, y: height, autoAlpha: 1 });
		TweenMax.set(['#copy-2'], { x: 0, y: height, autoAlpha: 0 });
		TweenMax.set(['#logo', "#logo2","#copy-3", "#copy-4"], { x: 0, autoAlpha: 0 });
		...
		click_through.onmouseover = function () {
			TweenMax.to("#cta", 0.1, { scale: 1.05, y: 0, transformOrigin: "88% 60%", rotationZ: 0.01, force3D: true, rotationZ: 0.01, transformPerspective: 400 });
		};
	...
	nameSpace.initAnimation = function () {
		...
		.to(["#copy-1"], 0.7, { y: 0, transformPerspective: 400, autoAlpha: 1, force3D: true, rotationZ: 0.01, ease: Linear.easeInOut })

		// .to(["#copy-1"], 0.4, { transformPerspective: 400, autoAlpha: 0, force3D: true, rotationZ: 0.01, ease: Linear.easeInOut }, "+=2.4")
	
		.to(["#copy-2"], 0.7, { y: 0, transformPerspective: 400, autoAlpha: 1, force3D: true, rotationZ: 0.01, ease: Linear.easeInOut }, "+=1")
		...
		.to(["#copy-1", "#copy-2", "#bg-img"], 0, { transformPerspective: 400, autoAlpha: 0, force3D: true, rotationZ: 0.01, ease: Power2.easeIn }, "+=3")
			...
			.to('#drawer', 0.5, {x:419, transformPerspective: 400, autoAlpha: 1, force3D: true, rotationZ: 0.01, ease: Power2.easeIn })


			.to(["#cta", "#code", "#logo", "#logo2"], 0, { autoAlpha: 1, ease: Cubic.easeInOut }, "+=0.5");
	};
	...
	nameSpace.startBgImg = function() {
		...
		t.to('#bg-img', 2.6, {scale: 1, x:-10 ,y:-20, transformPerspective: 400, force3D: true, rotationZ: 0.01, ease: Linear.easeNone})
	}
